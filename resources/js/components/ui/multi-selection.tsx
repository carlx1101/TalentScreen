									...
									<div className="flex w-max gap-1">
										{selected.map((item) => (
											<Badge
												key={item.value}
												variant="default"
												className="flex-shrink rounded-sm text-[13.6px] font-medium capitalize bg-white text-black dark:bg-black dark:text-white"
											>
												{item.label}
												<span
	...
	return (
		...
		>
			<CommandInput
				...
				className="placeholder:Capitalize h-full border-0 sticky top-0 z-10 bg-background"
				placeholder="Search..."
			/>
